...
All `@more-cheese-demo/*` packages version **together** (fixed versioning): the
`.changeset/config.json` `"fixed": [["@more-cheese-demo/*"]]` entry makes
...
|---|---|---|---|
| A **sibling package of this app** (e.g. `@more-cheese-demo/entities` from `Server`) | `dependencies` | **exact** (`1.0.0`) | Siblings ship in lock-step (fixed versioning); an exact pin means an install always gets the matched set |
