# Schema Reconciliation — Consolidated Walk-List (the workstream lead ↔ the schema owner)
...
| # | Ask | What we need from the schema owner | Source |
|---|---|---|---|
...
| B2 | **Pre-baked lat/long on people + orgs** | ⚠ cross-app ask: `Address` lives in **bizapps-common** — adding geocode columns (or a side table) touches the schema owner's shared app, not our cheese schemas. Member map is the demo's first visual; no live geocoding at install | GAP-11(a) |
...
| B4 | **Donation order-line typing + seed config** | Per the domain lead's GAP-7 ruling: donations ride Orders as POST-INSTALL config — demo ships donation ProductCategory/products/GL wiring as seed data. Confirm the orders/accounting schemas accept that shape | GAP-7, JSON `$donations_home` |
...
- **bizapps-forms (D10)**: if Pranav's schema is ready (the domain lead chasing status + freeze date), forms joins as composed app #11 with an optional data pack — confirm its table shapes alongside the others.
- **Readiness dates** for the other 8 composed apps' schema freezes (OQ-11) — the schema owner's survey answers this; the whole timeline hangs on it.
- **GAP-12** (calendar-year renewals vs release-relative hero pins) — decision may land here or at the R5 workshop, wherever the domain lead is in the room; see `gaps-to-fill.md` GAP-12 for the three options.
...
- the schema owner's BizApps schema survey (his half — not in this repo as of 2026-07-07)
