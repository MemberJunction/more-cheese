// Gate helpers — the shapes the bespoke validator checks kept re-implementing.
//
// fkResolves used to live here: one gate over a batch of foreign-key relations. It is gone, and
// nothing replaced it, because reference integrity is now DECLARED in each project's refs.mjs and
// the gates are generated from the declarations. A helper that makes it easy to hand-write a
// duplicate of a generated gate is not a convenience.
...
export function makeGateHelpers(check) {
  ...
  return { dangling, shareBand, presenceFloor, distinctAtLeast };
}
