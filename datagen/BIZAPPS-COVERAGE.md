...
| **bizapps-orders** | *(not targeted)* | — | **0** — app is pre-implementation | `morecheese_orders` is the **sanctioned stand-in** (the schema owner memo §2.4): Product/Order/OrderLine/Payment in our shapes until their Subscription + renewal-Order tables exist; then period rows decompose into their model |
...
**Not targeted, by ruling (the workstream lead, 2026-07-22 — scope narrowed for a binary "done"):**
...
2. **Install** — all 7 packs load green onto a cloned real MJ database; **every FK
   constraint reports trusted** (SQL Server validated each row). ⚠ That install was the
   2026-07-14 verification and covered the 7 packs shipping then. **Twelve** ship now —
   `messaging`, `platform` and `sonar` arrived later, and `common` gained addresses and
   contact methods. Their shapes are gated in the suite, but this *install* evidence has not
   been re-captured for them; re-running it is the honest way to restore this claim.
