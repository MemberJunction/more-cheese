...
P();
P(`One Sonar model — **${R.sonar.model.name}** (\`${R.sonar.model.slug}\`, ${R.sonar.model.status}) — scores every member on the ${R.sonar.model.anchorEntityName} spine. **Definitions only**: Sonar's FactorCompiler compiles the factors to SQL and computes the scores live, so we ship the model, not pre-computed numbers.`);
P();
P(`- **${R.sonar.factors.length} executable factors**, each a Declarative \`Count\` over a related entity linked by SourceRelatedEntityID (RelationshipPath \`[]\` → the compiler auto-resolves the FK path to the anchor): ${R.sonar.factors.map((f) => f.name).join(' · ')}.`);
