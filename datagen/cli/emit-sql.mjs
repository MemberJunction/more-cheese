...
const { MAPPING, INSTALL_ORDER, PREAMBLE, POSTAMBLE, DISPLAY_NAME } = await import(`../projects/${run.project}/seed-mapping.mjs`);
...
for (const pack of INSTALL_ORDER) {
  packIndex++;
  const lines = [
    `-- ${DISPLAY_NAME ?? run.project} demo seed — pack: ${pack} (install order ${packIndex})`,
