...
//
// MOVED OUT OF cli/ (2026-08-05). This is a DEV SHIM listing MoreCheese's own schemas and the
// bizapps stand-ins it needs for a throwaway playground database. Every table name in it belongs to
// this project, so it belongs to this project. The engine-boundary checker had to allowlist it while
// it lived in cli/.
...
const TABLES = [
  ...
  // hard FKs into the dependency schema (the schema owner's linking ruling)
...
const lines = [
  ...
  `-- Generated by datagen/projects/morecheese/emit-schema.mjs · seed ${run.seed} · release ${run.releaseDate} · ruleset v${run.ruleset}`,
  '-- ⚠ NOT the reconciliation. Run this FIRST, then the numbered seed packs (01_common.sql … ).',
  '--   When the schema owner\'s authoritative migrations land, discard this and load the packs into the real tables.',
