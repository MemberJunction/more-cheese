...
`projects/morecheese/` is the PROJECT — one generated universe: its domain modules (world,
membership, events, money, hooks), its name banks, and its `ruleset/`. A second universe =
a second `projects/<name>/` directory; the engine and entrypoints are shared (`--project`). **Every calibrated decision now runs through the five
...
| `generate.mjs` | Project-generic entrypoint: loads `projects/<p>/index.mjs`, calls its `buildWorld(cfg)`, emits packs. |
