...
Fix every FAIL and every ⚠ warning it prints (thin or undated research, homepage sources, copied source lists, real organisations named, missing member voice, British spellings, slug length) and re-run until the output shows no FAIL and no ⚠. If you must discard a draft, remove it with rm (only under content/blog/). Before choosing topics, read the titles of every post already in content/blog/ within four weeks of this one and do not repeat a theme or re-spotlight the same hero; a hero may only appear after their JoinDate in generated/member-profiles, and each hero gets at most one Member Spotlight across the whole corpus (grep content/blog for existing spotlights first). Do not edit posts from other weeks. Do not run git. Finish with the skill's §8 completion checklist."
start=$(date +%s)
