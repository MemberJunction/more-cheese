...
|---|---|---|
| [`packages/Entities`](packages/Entities) | `@mj-biz-apps/more-cheese-entities` | Strongly-typed TypeScript entity subclasses generated by CodeGen. |
| [`packages/Server`](packages/Server) | `@mj-biz-apps/more-cheese-server` | Server bootstrap package loaded by MJAPI at startup via `startupExport`. |
| [`packages/Angular`](packages/Angular) | `@mj-biz-apps/more-cheese-ng` | Angular client package bundled into MJExplorer; provides generated entity forms. |
...
A release is one dispatch and one merge: dispatch **Prepare a release**, review and
merge the pull request it opens into `main`, and `publish.yml` builds, publishes to
npm, tags `vX.Y.Z` and carries `main` back into `next` on its own. Nothing is ever
pushed to `main` or `next`.

```sh
npm run release:plan   # read-only preflight: is a release due, and what blocks it?
```

**[`docs/release.md`](docs/release.md)** is the runbook — the steps, the preflight,
and the failure-mode table for a red run.
